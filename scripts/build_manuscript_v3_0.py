...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_0.docx'
...
add_table(doc, ['Rate Mbps','RTT ms','Raw compl. ms','RL compl. ms','Raw app goodput Mbps','RL app goodput Mbps'], rows, widths=[1.0,0.8,1.1,1.1,1.3,1.3], font_size=7)
...
add_table(doc, ['Approach','E2E-encrypted transport','Per-reference auth','Fail-closed repair','Receiver dictionary','Best-fit workload'], [
    ...
    ['HTTP shared dictionary / CDT [19-22]','Yes (HTTP/3 on QUIC)','No (TLS channel only)','No (codec fallback)','Prior HTTP response','HTTP responses with prior versions'],
...
doc.core_properties.comments='ReduLink journal-ready v3.0 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
