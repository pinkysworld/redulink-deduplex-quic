...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_1.docx'
...
para(doc,"Table 13 reports three repeated trials of the native demo workload. The multiplier is deterministic for fixed bytes, while elapsed times vary modestly across runs; reporting both avoids cherry-picking a single favorable QUIC run; with only three trials these are run-to-run stability indicators rather than a statistically powered variance estimate. Table 14 then scales the payload from 96 to 1024 blocks: the stream multiplier rises from 3.19x to 3.87x as more reusable chunks accumulate, the miss count grows proportionally with the dictionary warm-up, and reconstruction remains byte-exact at every scale.")
sel=[]
for r in repeat:
...
tcap(doc,'Table 18. Analytic fluid-model completion times and implied application rates from measured encoded stream bytes (raw versus ReduLink); computed, not measured.')
...
para(doc,"Table 20 consolidates the positioning. ReduLink is the only approach in the comparison that combines operation over an end-to-end encrypted QUIC stream with per-reference authentication and an explicit fail-closed repair path; the other approaches each win in their own regime, which is exactly why ReduLink is presented as a complementary representation layer rather than a replacement.")
tcap(doc,'Table 20. Positioning of ReduLink relative to related redundancy-suppression approaches.')
...
para(doc,"The implementation maps ReduLink into QUIC streams rather than adding custom QUIC extension frames or transport parameters, and it uses an exporter-style key schedule model rather than live QUIC TLS exporter bytes. The public object-sequence and layer-like workloads are derived from real release bytes but are transfer-model experiments rather than captured production traces. The fairness evidence (competing-flow Jain indices and fluid bottleneck emulation in Section 10) is conservative and does not replace a full network-emulator congestion-control study, and all transport experiments run over localhost with a deterministic loss proxy rather than over a real path with diverse RTT, bandwidth, and loss. Every native-QUIC experiment uses at most 1 MiB over a single connection (most about 96 KiB); the byte multipliers are size-independent by construction, but completion time, miss dynamics, dictionary eviction, and flow-control and fairness interactions are not, and are out of scope at this scale. The security evidence is functional artifact testing rather than a formal or adversarial analysis. Absolute timings are hardware-dependent.")
...
doc.core_properties.comments='ReduLink journal-ready v3.1 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
