...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_2.docx'
...
para(doc,"Table 18 reports a measured concurrent competing-flow experiment in which a raw QUIC flow and a ReduLink flow run simultaneously over a shared localhost schedule with periodic datagram loss, repeated over "+str(compflow['rounds'])+" rounds. Across the measured rounds the reconstructed-rate Jain index [25] is "+f3(compflow['reconstructed_rate_jain_index'])+" and the encoded-rate index is "+f3(compflow['encoded_rate_jain_index'])+", with every flow reconstructing correctly; the lower encoded-rate index is expected, reflecting that ReduLink transmits far fewer encoded bytes than the raw flow rather than transport-level unfairness. This is a real loopback measurement, not a congestion-control study; kernel-level path emulation (tc/netem or Mininet) on a privileged host remains future work. Table 19 is an analytic illustration rather than a measurement: it applies a closed-form fluid model, completion time approximately equal to encoded_bytes times 8 divided by rate, plus two times RTT, to the measured encoded byte counts. Because ReduLink encodes fewer bytes, the model yields a lower completion time and therefore a higher implied application rate at the same encoded fair share; for example at 25 Mbps and 10 ms the model gives "+f1(29.722)+" ms versus "+f1(51.318)+" ms for raw. This advantage is arithmetic, a direct consequence of sending fewer encoded bytes, and is not evidence of fair coexistence under a real congestion controller; the modeled benefit narrows on the RTT-bound 50 ms rows, where latency rather than encoded volume dominates.")
...
tcap(doc,'Table 19. Analytic fluid-model completion times and implied application rates from measured encoded stream bytes (raw versus ReduLink); computed, not measured.')
...
para(doc,"Table 21 consolidates the positioning. ReduLink is the only approach in the comparison that combines operation over an end-to-end encrypted QUIC stream with per-reference authentication and an explicit fail-closed repair path; the other approaches each win in their own regime, which is exactly why ReduLink is presented as a complementary representation layer rather than a replacement.")
tcap(doc,'Table 21. Positioning of ReduLink relative to related redundancy-suppression approaches.')
...
doc.core_properties.comments='ReduLink journal-ready v3.2 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
