...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_6.docx'
...
tcap(doc,'Table 15. Repeated native QUIC trials (n = 3): run-to-run stability of multipliers and elapsed time.')
...
tcap(doc,'Table 19. Measured concurrent competing-flow experiment (Jain fairness index, twelve rounds).')
add_table(doc, ['Metric','Value'], [
...
tcap(doc,'Table 20. Measured shared-bottleneck path emulation (userspace token bucket + one-way delay; three rounds per point): completion, queueing, and encoded-rate fairness for competing raw and ReduLink flows.')
...
tcap(doc,'Table 21. Analytic fluid-model completion times and implied application rates from measured encoded stream bytes (raw versus ReduLink); computed, not measured.')
...
para(doc,"Table 23 consolidates the positioning. ReduLink is the only approach in the comparison that combines operation over an end-to-end encrypted QUIC stream with per-reference authentication and an explicit fail-closed repair path; the other approaches each win in their own regime, which is exactly why ReduLink is presented as a complementary representation layer rather than a replacement.")
tcap(doc,'Table 23. Positioning of ReduLink relative to related redundancy-suppression approaches.')
...
"[15] M. Nguyen, 'ReduLink artifact and reproducibility package,' tagged release v3.6-journal-submission, GitHub, 2026. https://github.com/pinkysworld/redulink-deduplex-quic/releases/tag/v3.6-journal-submission",
...
doc.core_properties.comments='ReduLink journal-ready v3.6 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
