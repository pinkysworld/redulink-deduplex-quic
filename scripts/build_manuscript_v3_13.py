...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_13.docx'
...
para(doc,"Scope note: Section 10 provides artifact-level emulation evidence, not transport-fairness proof. The userspace shaper and localhost competing-flow tests show that ReduLink is accounted on encoded bytes and that completion time depends on rate, RTT, and repair behavior. Section 10 now also includes a live kernel tc/netem sweep (Table 23) on a single-queue loopback path; it does not yet prove WAN performance, production congestion-control fairness with independent competing flows, Mininet behavior, or Internet-path deployment behavior.")
...
add_table(doc, ['Metric','Value'], [
    ...
    ['Scope', 'localhost unshaped measured experiment; kernel tc/netem is reported separately in Table 23'],
...
para(doc,"Table 23 reports the load-bearing result this section previously deferred: a live Linux tc/netem kernel-path sweep. Because the build environment grants no host NET_ADMIN, we run the harness inside a rootless user-plus-network namespace (unshare -rn), where tc/netem qdiscs are configurable without host privileges; the paired raw and ReduLink aioquic flows then traverse a real kernel netem qdisc on loopback (rate plus one-way delay), twenty rounds per demo point and twelve per Redis point, with deterministic bootstrap confidence intervals. The result is measured, not modeled, and it is deliberately unflattering. On this real kernel path ReduLink does not complete faster; it completes 1.08x (demo, 20 Mbps) to 1.60x (demo, 5 Mbps) as long as the raw flow, and 1.30x to 1.36x on the real Redis-layered bytes, even though it puts only 0.26 to 0.31 of the raw encoded bytes on the wire. The mechanism is the qdisc topology: a single netem qdisc on loopback is an aggregate, effectively half-duplex, shared pipe, so ReduLink's reverse MISS and repair round trips queue behind the forward stream and their added latency outweighs the byte savings for these small warm-update transfers, worst when the pipe is tightest. This directly confirms the caution we raised from the userspace emulation: our full-duplex userspace shaper had shown ReduLink completing faster (0.65x), and a real kernel shared pipe reverses that ordering, so we withdraw any completion-time advantage claim on constrained shared paths. What survives, and is confirmed on the kernel path, is the accounting invariant: ReduLink is charged strictly fewer encoded bytes (Table 18), and reconstruction stays byte-exact in every round. A full congestion-fairness study with independent competing flows, larger transfers, and a full-duplex or multi-queue topology remains future work, but the kernel-path completion-time behavior is now measured rather than assumed.")
rows=[]
...
tcap(doc,'Table 23. Live Linux tc/netem kernel-path emulation (rootless network namespace; single shared loopback qdisc): paired raw versus ReduLink completion ratio with bootstrap 95% CIs. Values above 1.0x mean ReduLink is slower.')
...
para(doc,"The package includes reviewer-runnable macOS dummynet and Linux tc/netem harnesses, and now a completed live Linux tc/netem loopback result (Table 23); a full Mininet or multi-host congestion-control study with independent competing flows remains future work. The competing-flow and bottleneck results are therefore deliberately conservative emulations that isolate the encoded-byte accounting principle rather than claim production fairness.")
...
para(doc,"The implementation maps ReduLink into QUIC streams rather than adding custom QUIC extension frames or transport parameters, and it uses an exporter-style key schedule model rather than live QUIC TLS exporter bytes. The public object-sequence and layer-like workloads are derived from real release bytes but are transfer-model experiments rather than captured production traces. The fairness evidence is a measured 20-round competing-flow experiment plus a measured 20-round full-duplex userspace path emulation across a rate/RTT grid on both byte-stable and real repair-heavy payloads (Section 10); userspace shaping is faithful at the studied rates but is not kernel tc/netem or Mininet, and all transport experiments run over localhost. We report a completed live Linux tc/netem kernel-path sweep (Table 23), run inside a rootless user-plus-network namespace so no host privileges are required; on that real single-queue kernel path ReduLink is slower for small warm-update transfers (completion ratio 1.08x to 1.60x), which we report rather than hide. The macOS pf/dnctl dummynet sweep still times out on our development Mac and remains future work. The path-emulation history is a caution in itself: our first single-bucket userspace shaper produced a completion-time finding that a full-duplex userspace model reversed, and the live single-queue kernel netem path reversed it again toward ReduLink being slower; we therefore make no completion-time advantage claim and treat the encoded-byte accounting, not latency, as the transport contribution. A stronger transport-performance claim should therefore be deferred until privileged-host kernel emulation, packet capture, and competing production-style flows reproduce the encoded-byte accounting under real congestion control. The miss-rate sweep is a one-path sensitivity check, not a full transport parameter study. Native-QUIC experiments now reach 16 MiB over a single localhost connection, including a demonstrated dictionary-budget overflow and recovery; multi-connection behavior, larger-than-memory dictionaries, and Internet-path dynamics remain out of scope. The independent positive version-pair study of Section 7.5 captures real package-distribution objects rather than a live production registry pull. The security analysis of Section 4.5 reduces reference unforgeability to standard HMAC and collision-resistance assumptions, but a machine-checked proof and production exporter-derived keying on every path remain future work. Headline byte multipliers are model-framed; the wire-priced values of Section 7.6 are 5 to 28 percent lower, and whole-stream dictionary delta beats both wherever its retention and trust assumptions hold. Absolute timings are hardware-dependent.")
para(doc,"Future work should add custom QUIC frame integration and transport-parameter negotiation, live exporter-derived keys, production replay-window and cross-tenant isolation enforcement, a full-duplex or multi-queue kernel fairness study with independent competing flows (beyond the single-queue loopback netem result of Table 23), packet capture, larger and live-captured registry or backup traces, a broader miss-rate grid across bandwidth and RTT, a machine-checked security proof, and a direct head-to-head comparison against Compression Dictionary Transport on the same corpora. The current zstd --patch-from baseline is intentionally the closest byte-level CDT analog available in the artifact; an HTTP-level CDT comparison would require serving the same object corpus through a CDT-capable HTTP stack.")

heading(doc,'14. Reproducibility',1)
para(doc,"The package contains source code, public corpora, generated corpora, benchmark scripts, result CSV/JSON files, figures, a citation checker, and tests [15]. The reviewer-facing smoke command is python3 scripts/run_smoke_validation.py and full validation is python3 scripts/run_full_validation.py. aioquic tests skip gracefully when aioquic is unavailable; installing requirements-dev.txt enables complete QUIC stream validation, while requirements-lock.txt and the repository Dockerfile provide an exact dependency path for reviewers who want a pinned container run. Every table and figure in this paper is regenerated from the committed result files by scripts/build_manuscript_v3_12.py and scripts/make_journal_figures_v2_8.py. The framing-repricing and dictionary-delta baseline is regenerated by benchmarks/run_framing_dictionary_baseline.py, the miss-rate sensitivity sweep by benchmarks/run_quic_miss_rate_sensitivity.py, the QUIC confidence-interval table by benchmarks/summarize_quic_statistical_evidence.py, and the verifier-hardening properties of Section 4.5 (authentication-first ordering, bounded replay window, context binding under a valid tag) are pinned by dedicated unit tests. Each version of this manuscript is accompanied by an annotated git tag and a GitHub release whose assets include the final PDF and DOCX; the SHA-256 hashes of both files are pinned in MANUSCRIPT_SHA256.txt in the tagged tree, so reviewers can verify that the reviewed PDF matches the archived artifact. The repository also includes PUBLIC_REVIEWER_CHECKLIST.md and scripts/verify_public_release.py, which check the unauthenticated latest-release API, tag-level raw files, branch raw files, and exact commit raw files when a cached GitHub HTML or raw view appears stale.")
...
"[15] M. Nguyen, 'ReduLink artifact and reproducibility package,' tagged release v3.13-journal-submission, GitHub, 2026. https://github.com/pinkysworld/redulink-deduplex-quic/releases/tag/v3.13-journal-submission",
...
doc.core_properties.comments='ReduLink journal-ready v3.13 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
