...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_5.docx'
...
pypi = read_csv('pypi_object_trace.csv')
...
para(doc,"Table 6 reports the deterministic warm-state fixtures. On most byte-stable workloads, fixed-block reuse matches or beats ReduLink; the distinct ReduLink contribution is not superior matching but authenticated, scoped, stream-compatible reference substitution with explicit repair and privacy rules. The negative rows (package metadata, logs, and the compressed control) are included precisely because they fail: where repetition is local to one object, compression wins, and where there is no reusable structure, both fixed reuse and ReduLink correctly decline to help. The Unchanged column makes the mechanism explicit: because the effective multiplier is bounded by roughly one over one minus the unchanged fraction, these deterministic fixtures (96 to 100 percent unchanged on the positive rows) are illustrative workload shapes chosen to expose where reference substitution can and cannot help, not estimates of real-world overlap. The repository fixture in particular changes only 210 of 501,760 bytes, so its 24.73x is close to a no-op transfer.")
...
tcap(doc,'Table 11. Independent captured positive trace: real PyPI package upgrades (object-aligned, hash-pinned).')
...
para(doc,"Table 12 compares raw and ReduLink stream mapping at zero and periodic datagram loss, and Table 13 reports positive and negative workload cases. ReduLink reconstructs byte-exactly in every case, including the lossy proxy path, and the compressed-negative control correctly yields no gain. Figure 4 plots the workload cases. Because ReduLink writes fewer stream-payload bytes, its UDP-estimated multiplier stays above one on positive workloads even after the IPv4/UDP per-datagram overhead is added.")
rows=[]
for r in quic_flow:
...
tcap(doc,'Table 12. Native aioquic stream mapping: raw versus ReduLink under zero and periodic datagram loss.')
...
tcap(doc,'Table 13. Native aioquic stream mapping across positive and negative workload cases.')
...
para(doc,"Table 14 reports three repeated trials of the native demo workload. The multiplier is deterministic for fixed bytes, while elapsed times vary modestly across runs; reporting both avoids cherry-picking a single favorable QUIC run; with only three trials these are run-to-run stability indicators rather than a statistically powered variance estimate. Table 15 then scales the payload from 96 to 16,384 blocks (96 KiB to 16 MiB): the stream multiplier rises from 3.19x to 3.94x as more reusable chunks accumulate, and reconstruction remains byte-exact at every scale. The 16 MiB rows expose an operating constraint that smaller runs cannot show: with the artifact's default dictionary budget of 8,192 chunks, a 16,384-chunk warm set overflows the LRU, sequential FULL admissions cascade-evict the remaining warm entries, and ReduLink degrades to a FULL-only stream (0.92x, pure framing overhead) while still reconstructing byte-exactly, i.e. it fails safe rather than wrong. Raising the budget to 24,576 chunks restores 3.96x and roughly halves completion time. Dictionary sizing relative to the warm working set is therefore a hard provisioning requirement, not a tuning nicety.")
sel=[]
for r in repeat:
...
tcap(doc,'Table 14. Repeated native QUIC trials (n = 3): run-to-run stability of multipliers and elapsed time.')
...
tcap(doc,'Table 15. Payload scaling of the native aioquic stream mapping, including the dictionary-budget overflow and recovery at 16 MiB.')
...
tcap(doc,'Table 16. Component-level cost measurements (local Python artifact timing; not production line-rate).')
...
para(doc,"Table 18 reports a measured concurrent competing-flow experiment in which a raw QUIC flow and a ReduLink flow run simultaneously over a shared localhost schedule with periodic datagram loss, repeated over "+str(compflow['rounds'])+" rounds. Across the measured rounds the reconstructed-rate Jain index [25] is "+f3(compflow['reconstructed_rate_jain_index'])+" and the encoded-rate index is "+f3(compflow['encoded_rate_jain_index'])+" (for two concurrent flows Jain's index ranges from 0.5 to 1.0), with every flow reconstructing correctly; the lower encoded-rate index is expected, reflecting that ReduLink transmits far fewer encoded bytes than the raw flow rather than transport-level unfairness. This is a real loopback measurement without rate shaping; Table 19 adds measured rate and delay emulation. Table 20 is an analytic illustration rather than a measurement: it applies a closed-form fluid model, completion time approximately equal to encoded_bytes times 8 divided by rate, plus two times RTT, to the measured encoded byte counts. Because ReduLink encodes fewer bytes, the model yields a lower completion time and therefore a higher implied application rate at the same encoded fair share; for example at 25 Mbps and 10 ms the model gives "+f1(29.722)+" ms versus "+f1(51.318)+" ms for raw. This advantage is arithmetic, a direct consequence of sending fewer encoded bytes; the measured emulation of Table 19 shows why it is optimistic, since the fluid model ignores queueing delay and ReduLink's application-level repair round trips, which in measurement reverse the completion-time ordering at 5 Mbps. We retain the model only as a best-case bound on the byte-volume effect.")
tcap(doc,'Table 18. Measured concurrent competing-flow experiment (Jain fairness index, twelve rounds).')
add_table(doc, ['Metric','Value'], [
...
para(doc,"Table 19 adds rate and delay to the same harness as a measured emulation: both flows run concurrently through localhost proxies that share a single userspace token-bucket link with one-way propagation delay, sweeping 5 and 20 Mbps against 20 and 80 ms RTT with three rounds per point. This is a real measurement of aioquic traffic through an emulated bottleneck, though userspace shaping is not kernel netem. The result is deliberately unflattering and, we argue, the most important fairness finding in the paper: despite transmitting 3.19x fewer encoded bytes, ReduLink does not complete faster, and at 5 Mbps with 20 ms RTT it completes about 74 percent slower than the competing raw flow (1275 ms versus 731 ms). The instrumented shaper explains the mechanism: at 5 Mbps the shared queue contributes about 24 ms of mean queueing delay (72 ms maximum) behind the raw flow's larger byte volume, and ReduLink's semantic MISS/repair exchange must cross that inflated path more times than a one-way bulk stream, whereas at 20 Mbps, where measured queueing is about 1 ms, the two flows complete within 3 to 10 percent of each other. Reconstruction is byte-exact in every round and encoded-rate Jain indices stay between 0.67 and 0.78. The sharpened claim boundary follows: on constrained shared paths, ReduLink's measured benefit is byte-cost reduction at equal congestion fairness, not single-transfer latency; completion-time benefits require miss-free warm state or pipelined bulk transfers.")
rows=[]
for sm in empath['summary']:
    ratio = sm['redulink_completion_ms_mean']/sm['raw_completion_ms_mean'] if sm['raw_completion_ms_mean'] else 0
    rows.append([f1(sm['rate_mbps']), f1(sm['rtt_ms']), f1(sm['raw_completion_ms_mean']), f1(sm['redulink_completion_ms_mean']), f"{ratio:.2f}x", f1(sm['mean_queue_delay_ms']), f3(sm['encoded_rate_jain_index']), 'OK' if sm['all_reconstructed'] else 'FAIL'])
tcap(doc,'Table 19. Measured shared-bottleneck path emulation (userspace token bucket + one-way delay; three rounds per point): completion, queueing, and encoded-rate fairness for competing raw and ReduLink flows.')
add_table(doc, ['Rate Mbps','RTT ms','Raw ms','ReduLink ms','RL/raw','Mean queue ms','Jain (enc)','Recon.'], rows, widths=[0.8,0.7,0.8,1.0,0.7,1.0,0.8,0.6], font_size=7)
...
tcap(doc,'Table 20. Analytic fluid-model completion times and implied application rates from measured encoded stream bytes (raw versus ReduLink); computed, not measured.')
...
para(doc,"Table 22 consolidates the positioning. ReduLink is the only approach in the comparison that combines operation over an end-to-end encrypted QUIC stream with per-reference authentication and an explicit fail-closed repair path; the other approaches each win in their own regime, which is exactly why ReduLink is presented as a complementary representation layer rather than a replacement.")
tcap(doc,'Table 22. Positioning of ReduLink relative to related redundancy-suppression approaches.')
...
para(doc,"The implementation maps ReduLink into QUIC streams rather than adding custom QUIC extension frames or transport parameters, and it uses an exporter-style key schedule model rather than live QUIC TLS exporter bytes. The public object-sequence and layer-like workloads are derived from real release bytes but are transfer-model experiments rather than captured production traces. The fairness evidence is a measured twelve-round competing-flow experiment plus a measured userspace path emulation across a rate/RTT grid (Section 10); userspace shaping is faithful at the studied rates but is not kernel tc/netem or Mininet, which requires a privileged host and remains future work, and all transport experiments run over localhost. Native-QUIC experiments now reach 16 MiB over a single localhost connection, including a demonstrated dictionary-budget overflow and recovery; multi-connection behavior, larger-than-memory dictionaries, and Internet-path dynamics remain out of scope. The independent positive trace of Section 7.5 captures real package-distribution objects rather than a live production registry pull. The security analysis of Section 4.5 reduces reference unforgeability to standard HMAC and collision-resistance assumptions, but a machine-checked proof and production exporter-derived keying remain future work. Absolute timings are hardware-dependent.")
para(doc,"Future work should add custom QUIC frame integration and transport-parameter negotiation, live exporter-derived keys, production replay-window and cross-tenant isolation enforcement, a privileged-host tc/netem or Mininet fairness study with competing real flows, packet capture, larger and live-captured registry or backup traces, a machine-checked security proof, and a direct head-to-head comparison against Compression Dictionary Transport on the same corpora.")

heading(doc,'14. Reproducibility',1)
para(doc,"The package contains source code, public corpora, generated corpora, benchmark scripts, result CSV/JSON files, figures, a citation checker, and tests [15]. The reviewer-facing smoke command is python3 scripts/run_smoke_validation.py and full validation is python3 scripts/run_full_validation.py. aioquic tests skip gracefully when aioquic is unavailable; installing requirements-dev.txt enables complete QUIC stream validation. Every table and figure in this paper is regenerated from the committed result files by scripts/build_manuscript_v3_5.py and scripts/make_journal_figures_v2_8.py. Each version of this manuscript is accompanied by an annotated git tag and a GitHub release whose assets include the final PDF and DOCX; the SHA-256 hashes of both files are pinned in MANUSCRIPT_SHA256.txt in the tagged tree, so reviewers can verify that the reviewed PDF matches the archived artifact.")
...
"[15] M. Nguyen, 'ReduLink artifact and reproducibility package,' tagged release v3.5-journal-submission, GitHub, 2026. https://github.com/pinkysworld/redulink-deduplex-quic/releases/tag/v3.5-journal-submission",
...
doc.core_properties.comments='ReduLink journal-ready v3.5 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
