...
OUT = ROOT/'paper/submission/ReduLink_journal_ready_v3_8.docx'
...
tcap(doc,'Table 22. Analytic fluid-model completion times and implied application rates from measured encoded stream bytes (raw versus ReduLink); computed, not measured.')
...
para(doc,"Table 24 consolidates the positioning. ReduLink is the only approach in the comparison that combines operation over an end-to-end encrypted QUIC stream with per-reference authentication and an explicit fail-closed repair path; the other approaches each win in their own regime, which is exactly why ReduLink is presented as a complementary representation layer rather than a replacement.")
tcap(doc,'Table 24. Positioning of ReduLink relative to related redundancy-suppression approaches.')
...
"[15] M. Nguyen, 'ReduLink artifact and reproducibility package,' tagged release v3.8-journal-submission, GitHub, 2026. https://github.com/pinkysworld/redulink-deduplex-quic/releases/tag/v3.8-journal-submission",
...
doc.core_properties.comments='ReduLink journal-ready v3.8 manuscript; ORCID: 0000-0001-6834-4422; University of the People'
doc.save(OUT)
print('saved', OUT)
